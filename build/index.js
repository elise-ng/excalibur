...
async function login(page, username, password) {
  try {
    ...
    const response = await Promise.race([page.waitForResponse(URL_SIS_HOMEPAGE_FRAME, {
      timeout: TIMEOUT_2FA
...
async function main() {
  try {
    ...
    const result = await login(page, _config.default.username, _config.default.password);
